...
# These constants specify the maximum size of the board.
...
def _new_game_board(columns: int, rows: int) -> [[int]]:
    '''
...
def _board_columns(board: [[int]]) -> int:
    '''Returns the number of columns on the given game board'''
    return len(board)



def _board_rows(board: [[int]]) -> int:
...
def _require_valid_column_number(column_number: int, board: [[int]]) -> None:
...
def _require_valid_column_count(columns: int) -> None:
    ...
    if columns < 1 or columns > MAX_COLUMNS:
        raise ValueError(f'columns must be an int between 1 and {MAX_COLUMNS}')
...
def _require_valid_row_count(rows: int) -> None:
    ...
    if rows < 1 or rows > MAX_ROWS:
        raise ValueError(f'rows must be an int between 1 and {MAX_ROWS}')
